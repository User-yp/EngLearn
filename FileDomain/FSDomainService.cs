...
public class FSDomainService
{
    ...
    public async Task<(OperateResult, List<UploadedItem>?)> GetAllFileAsync()
    {
        var uploadItem = await repository.GetAllFileAsync();
        if (uploadItem.Count == 0)
            return (OperateResult.Failed(new OperateError { Code="InvalidItem",Description="NoItem"}),null);
        return (OperateResult.Success, uploadItem);
    }

    public async Task<(OperateResult, UploadedItem?)> FindFileByHashAsync(string sha256Hash)
    {
        var uploadItem = await repository.FindFileByHashAsync(sha256Hash);
        if (uploadItem == null)
            return (OperateResult.Failed(new OperateError { Code = "InvalidItem", Description = "NoItem" }), null);
        return (OperateResult.Success, uploadItem);
    }
    public async Task<(OperateResult, UploadedItem?)> FindFileByIdAsync(Guid Id)
    {
        var uploadItem = await repository.FindFileByIdAsync(Id);
        if (uploadItem == null)
            return (OperateResult.Failed(new OperateError { Code = "InvalidItem", Description = "NoItem" }), null);
        return (OperateResult.Success, uploadItem);

    }
    public async Task<(OperateResult, UploadedItem?)> FindFileByNameAsync(string fileName)
    {
        var uploadItem = await repository.FindFileByNameAsync(fileName);
        if (uploadItem == null)
            return (OperateResult.Failed(new OperateError { Code = "InvalidItem", Description = "NoItem" }), null);
        return (OperateResult.Success, uploadItem);

    }
    public async Task<UploadedItem> HandlerUploadAsync(Stream stream, string fileName, CancellationToken cancellationToken)
    {
        string hash = HashHelper.ComputeSha256Hash(stream);
        long fileSize = stream.Length;
        DateTime today = DateTime.Today;
        ...
        //DDD中尽量避免直接在领域服务中执行数据库的修改（包含删除、新增）操作。
        //return UploadedItem.Create(id, fileSize, fileName, hash, backupUrl, remoteUrl);
        return UploadedItem.Create(id, fileSize, fileName, hash, backupUrl, null);
    }
}
